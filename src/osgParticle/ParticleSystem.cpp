...
void osgParticle::ParticleSystem::ArrayData::dispatchArrays(osg::State& state)
{
    ...
    arrayDispatchers.activateVertexArray(vertices.get());
    arrayDispatchers.activateNormalArray(normals.get());
    arrayDispatchers.activateColorArray(colors.get());
    arrayDispatchers.activateTexCoordArray(0, texcoords2.get());
    arrayDispatchers.activateTexCoordArray(1, texcoords3.get());

    if (vertices.valid()) state.setVertexPointer(vertices.get());
    if (normals.valid()) state.setNormalPointer(normals.get());
    if (colors.valid()) state.setColorPointer(colors.get());
    if (texcoords2.valid()) state.setTexCoordPointer(0, texcoords2.get());
    if (texcoords3.valid()) state.setTexCoordPointer(1, texcoords3.get());
